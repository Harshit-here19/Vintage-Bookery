...
function setupEventListeners() {
    ...
    document.addEventListener('keydown', function(e) {
        if (!elements.readerScreen || !elements.readerScreen.classList.contains('active')) return;
...
function readPdfFile(file) {
    if (typeof pdfjsLib === 'undefined') {
        hideLoading();
        showError('PDF support requires internet connection. Please try a text file.');
        return;
    }
    ...
    reader.onload = function(e) {
        ...
        parseEpub(arrayBuffer, file.name)
            .then(function(result) {
                ...
                // Set metadata
                if (result.metadata) {
                    if (result.metadata.title && elements.bookTitle && !elements.bookTitle.value) {
                        elements.bookTitle.value = result.metadata.title;
                    }
                    if (result.metadata.creator && elements.authorName && !elements.authorName.value) {
        ...
        JSZip.loadAsync(arrayBuffer)
            ...
            .then(function(data) {
                ...
                data.chapters.forEach(function(chapter, idx) {
                    ...
                    const chapterTitle = extractChapterTitle(chapter.content) || 'Chapter ' + (idx + 1);
...
function extractEpubMetadata(opfDoc) {
    ...
    function getMetaText(names) {
        for (let i = 0; i < names.length; i++) {
            const el = metadataEl.querySelector(names[i]) ||
                       metadataEl.getElementsByTagName(names[i])[0] ||
                       metadataEl.querySelector('dc\\:' + names[i]) ||
                       metadataEl.getElementsByTagNameNS('http://purl.org/dc/elements/1.1/', names[i])[0];
...
function extractSpineOrder(opfDoc) {
    const spine = [];
    const spineEl = opfDoc.querySelector('spine') || opfDoc.getElementsByTagName('spine')[0];
...
function extractManifest(opfDoc) {
    const manifest = {};
    const manifestEl = opfDoc.querySelector('manifest') || opfDoc.getElementsByTagName('manifest')[0];
...
function extractTextFromHtml(htmlContent) {
    ...
    const unwanted = doc.querySelectorAll('script, style, noscript, nav, header, footer');
    ...
    let text = '';
    const textElements = body.querySelectorAll('p, h1, h2, h3, h4, h5, h6, div, li, blockquote, span');
...
function showEpubMetadata(result) {
    ...
    if (metadata.title) {
        html += '<div class="epub-metadata-item"><span class="epub-metadata-label">Title:</span><span class="epub-metadata-value">' + escapeHtml(metadata.title) + '</span></div>';
    }
    if (metadata.creator) {
        html += '<div class="epub-metadata-item"><span class="epub-metadata-label">Author:</span><span class="epub-metadata-value">' + escapeHtml(metadata.creator) + '</span></div>';
    }
    if (metadata.publisher) {
        html += '<div class="epub-metadata-item"><span class="epub-metadata-label">Publisher:</span><span class="epub-metadata-value">' + escapeHtml(metadata.publisher) + '</span></div>';
    }
    if (metadata.language) {
        html += '<div class="epub-metadata-item"><span class="epub-metadata-label">Language:</span><span class="epub-metadata-value">' + escapeHtml(metadata.language) + '</span></div>';
    }
    
    html += '<div class="epub-metadata-item"><span class="epub-metadata-label">Chapters:</span><span class="epub-metadata-value">' + chapters.length + '</span></div>';
    
    const charCount = result.text ? result.text.length : 0;
    html += '<div class="epub-metadata-item"><span class="epub-metadata-label">Characters:</span><span class="epub-metadata-value">' + charCount.toLocaleString() + '</span></div>';
    ...
    if (chapters.length > 0) {
        ...
        if (chapters.length > 10) {
            html += '<div class="epub-chapter-item" style="opacity:0.6;font-style:italic;">...and ' + (chapters.length - 10) + ' more chapters</div>';
        }
        html += '</div>';
    }
...
function showFileTypeIndicator(file) {
    ...
    if (!indicator && elements.dropZone) {
        ...
        indicator.querySelector('.file-remove').addEventListener('click', function(e) {
            e.stopPropagation();
            clearFileSelection();
...
function createBook() {
    ...
    state.text = text;
    state.title = (elements.bookTitle && elements.bookTitle.value.trim()) || 'Untitled Book';
    state.author = (elements.authorName && elements.authorName.value.trim()) || 'Unknown Author';
    state.currentPage = 0;
...
function generateTableOfContents() {
    ...
    if (epubState.chapters && epubState.chapters.length > 0) {
        epubState.chapters.forEach(function(chapter, idx) {
            ...
            item.innerHTML = 
                '<span class="toc-item-title">' + escapeHtml(chapter.title) + '</span>' +
                '<span class="toc-item-page">Page ' + (pageIndex + 1) + '</span>';
...
function renderPages() {
    ...
    if (elements.leftContent) {
        if (leftIdx < total) {
            ...
            elements.leftContent.innerHTML = '<p class="end-text" style="text-align:center;font-style:italic;color:var(--ink-faded);">— The End —</p>';
        }
    }
...
function displaySearchResults() {
    ...
    showSearchMessage('Found ' + results.length + ' result(s) for "' + searchState.searchTerm + '"');
...
function exportToPDF() {
    if (typeof jspdf === 'undefined' && typeof window.jspdf === 'undefined') {
        updateExportProgress(100, 'PDF library not loaded. Please refresh and try again.');
        return;
    }

    const jsPDF = window.jspdf.jsPDF;
    
    const pageSize = elements.exportPageSize ? elements.exportPageSize.value : 'a4';
    const includeCover = elements.exportIncludeCover ? elements.exportIncludeCover.checked : true;
    const vintageStyle = elements.exportVintageStyle ? elements.exportVintageStyle.checked : true;
    ...
    setTimeout(function() {
        const fileName = state.title.replace(/[^a-zA-Z0-9\s]/g, '').replace(/\s+/g, '_') + '.pdf';
        doc.save(fileName);
...
function exportToHTML() {
    ...
    if (vintageStyle) {
        html += 'body{font-family:Georgia,serif;background:#f4e4c1;color:#2c1810;max-width:800px;margin:0 auto;padding:40px;line-height:1.8;}\n';
        ...
        html += 'p:first-child::first-letter{font-size:3em;float:left;padding-right:10px;color:#8b0000;line-height:1;}\n';
    } else {
        html += 'body{font-family:Georgia,serif;max-width:800px;margin:0 auto;padding:40px;line-height:1.8;}\n';
